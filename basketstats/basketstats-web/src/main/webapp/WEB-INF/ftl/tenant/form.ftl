...
<div id="tenant-form" class="default-form clearfix">
	...
	<form method="post" action="process.html')}">
		...
		<label for="name"><@spring.messageText "tenant.key.label" "Key" /><span
			class="required"><@spring.messageText "form.requiredField" "*" /></span></label>
		<input type="text" id="key" name="key" placeholder="Tenant Key" required="required"
			autofocus="autofocus" value="${(tenant.key)!''}" />
